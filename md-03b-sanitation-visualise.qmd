...
```{r}
sanitation_AFG_00_20_not_sm <- sanitation %>% 
  filter(name == "Afghanistan", year == 2020 | year == 2000, varname_short != "san_sm")
...
```{r}
ggplot(sanitation_AFG_00_20_not_sm, aes(x=residence, y=percent, fill=varname_long)) +
  geom_col() +
  facet_wrap(~year)+
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? the colors of the bars and legend have the same order
2.  What would you want to change? I am not sure! perhaps order the legend in order of increasing service level
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? there seem to be a lot of NA values for this variable.

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot? the y-scale is inconsistent which can be misleading. there is no legend but the color does not add new information so it does not matter.
