...
5.  What data types are contained in the variables? (Reminder: [https://rbtl-fs24.github.io/website/slides/lec-02-visualisation.html#/types-of-variables](https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables){.uri}) factor, numeric, integer
...
```{r}
penguins_adelie_long <- penguins_adelie %>% 
...
```{r}
penguins_adelie_female_short <- penguins_adelie %>% 
...
```{r}
penguins_chinstrap_female_dream_torgersen <- penguins %>% 
  filter(species == "Chinstrap",
         sex == "female",
...
```{r}
penguins_gentoos_heavy <- penguins %>% 
...
```{r}
penguins_summary_min_max <- penguins %>% 
  group_by(species) %>% 
...
```{r}
penguins_summary_mean <- penguins %>% 
...
```{r}
penguins %>% 
  filter(sex == "female", island == "Biscoe" | island == "Torgersen") %>% 
  count()
```
